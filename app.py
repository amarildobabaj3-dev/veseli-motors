...
app = Flask(__name__)
app.secret_key = 'veseli_motors_final_fix'

# Konfigurimi i Databazës (E thjeshtuar maksimalisht)
basedir = os.path.abspath(os.path.dirname(__file__))
app.config['SQLALCHEMY_DATABASE_URI'] = 'sqlite:///' + os.path.join(basedir, 'motors.db')
...
# Kjo komande krijon databazen automatikisht
...
def shto():
    ...
    if request.method == 'POST':
        m = Makina(
            ...
            foto_2=request.form.get('foto2'), pershkrimi=request.form.get('pershkrimi')
        )
        db.session.add(m)
        db.session.commit()
